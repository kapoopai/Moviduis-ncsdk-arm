...
ncStatus_t ncDeviceOpen(struct ncDeviceHandle_t * deviceHandle)
{
...
#else
    Dl_info info;
    dladdr((const void*)ncDeviceOpen, &info);
    ...
    //printf("%s: %d, File path %s\n", __func__, __LINE__, mv_cmd_file_path);
                            ...
                            int option, void *data, unsigned int *dataLength)
{
    ...
    ncOptionClass_t opt_class = getOptionClass(option, GRAPH_CLASS0_BASE);
    if (g->dev != NULL && opt_class > g->dev->dev_attr.max_graph_opt_class) {
        mvLog(MVLOG_ERROR, "This device FW does not support NC_OPTION_CLASS%d",
              opt_class);
        return NC_UNAUTHORIZED;
    }
...
#ifndef EXCLUDE_HIGHCLASS
    case NC_OPTION_CLASS2:
        rc = getGraphOptionClass2(g, (ncGraphOption_t)option, data, dataLength);
        break;
    case NC_OPTION_CLASS3:
        rc = getGraphOptionClass3(g, (ncGraphOption_t)option, data, dataLength);
        break;
#endif
    default:
        mvLog(MVLOG_ERROR, "There is no such option class");
...
#ifndef EXCLUDE_HIGHCLASS
    case NC_OPTION_CLASS2:
        rc = getDeviceOptionClass2(d, (ncDeviceOption_t)option, data, dataLength);
        break;
    case NC_OPTION_CLASS3:
        rc = getDeviceOptionClass3(d, (ncDeviceOption_t)option, data, dataLength);
        break;
#endif
    default:
        rc = NC_INVALID_PARAMETERS;
                           ...
                           const void *data, unsigned int dataLength)
{
    ...
    case NC_RW_FIFO_DATA_TYPE:{
            ...
            f->host_tensor_desc.dataType = (ncFifoDataType_t)tempDType;
            break;
        }
