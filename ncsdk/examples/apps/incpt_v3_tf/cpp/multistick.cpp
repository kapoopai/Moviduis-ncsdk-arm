...
#define GOOGLENET_GRAPH_FILE_NAME "inception_v3.graph"
...
const int networkDimGoogleNet = 299;  // 使用Inc_v3时是299      //224;
...
float *LoadImage(const char *path, int reqSize, float *mean)
{
    ...
    for(i = 0; i < reqSize*reqSize; i++)
    {
        ...
        imgfp32[3*i+0] = (blue-128) / 128;	// blue-mean[0];
        imgfp32[3*i+1] = (green-128) / 128;	// green-mean[1];
        imgfp32[3*i+2] = (red-128) / 128;	// red-mean[2];
    }
    return imgfp32;
}
